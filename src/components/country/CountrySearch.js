  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className={
          "dark:bg-inputDark dark:placeholder-gray-300 dark:placeholder-opacity-40 dark:text-gray-300 dark:text-opacity-40 bg-gray-100 bg-opacity-60  text-gray-700 text-opacity-70 shadow-md focus:shadow-xl rounded py-4 pr-2 pl-12 w-full focus:outline-none transition duration-300 ease-in-out"
        }
      ...
      />
      {data && (
        <datalist id="countries" className="w-96 bg-red-600">
          {data.map((country) => {
            return (
              <option key={country.countryCode + country.country}>
                {country.country}
              </option>
            );
          })}
