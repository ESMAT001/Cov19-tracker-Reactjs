...
const Status = ({ totalConfirmed = false, data, color, preCls = "", children, delay = 1, duration = 1 }) => {
    ...
    // }
    return (
        <div className={preCls + ` dark:bg-${color}-800  dark:bg-opacity-30 capitalize text-${color}-600 tex-base sm:text-lg px-2 font-bold w-full text-center py-8 rounded hover:shadow-lg shadow transform hover:scale-105 bg-${color}-200 transition duration-300 ease-in-out`}>
            {children} <br />
